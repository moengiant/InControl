...
#Initialize AddOn
settings = xbmcaddon.Addon(id='incontrol.addon')
...
plugin_test = "Working"
plugin_start = "InCOntrol plugin started"
plugin_ip = "Please enter your InControl Server IP address in the addon settings"
plugin_port = "Please enter your InControl Server Port number in the addon settings"
plugin_user = "Please enter your InControl Server Username in the addon settings"
plugin_pword = "Please enter your InControl Server Password in the addon settings"
plugin_gid = "Please assign an InControl Group (Scene) ID in the addon settings"
plugin_auth_fail = "Please check your InControl Server Username and Password in the addon settings"
plugin_timeout = "Please check your InControl Server IP address and Port in the addon settings (Timeout)"
plugin_redirects = "InControl Server Communication: Failed - Too many redirects"
plugin_fail = "InControl Server Communication: Failed - Catastrophic Fail"
plugin_success = "InControl Server Communication: Successful"

#Notification messages - display time
time1        = 9000
time2        = 3000

#Initialize InControl Server information
...
#Initialize InControl Server Groups (Scenes)
...
#InControl Server communication and checks
def incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = "" ):
#Check if InControl Server information has been entered
    if (ip == "0.0.0.0"):
        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_ip, time1, __icon__))
        print("InControl Addon - Settings: Server IP Address Not Set")
    else:
        if (port == ""):
            xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_port, time1, __icon__))
            print("InControl Addon - Settings: Server Port Not Set")
        else:
            #if (username == ""):
            #    xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_user, time1, __icon__))
            #    print("InControl Addon - Settings: Server User Name Not Set")
            #else:
            #    if (password == ""):
            #        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_pword, time1, __icon__))
            #        print("InControl Addon - Settings: Server Password Not Set")
            #    else:
                    try:
                        auth=HTTPBasicAuth( username, password )
                        scenedata = {}
                        scenedata['password'] = password
                        scenedata['sceneName'] = command
                        scenedata['activate'] = 1
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,command,time2, __icon__))
                        url = 'http://%s:%s/zwave/activateScene' % (ip,port)
                        r = requests.put(url=url, json=scenedata)
                        s = str(r.status_code)
                        if (s == "200"):
                            if (degug_mode == "Yes"):
                                xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_success,time2, __icon__))
                                print("InControl Addon - Server Communication: Successful")
                        else:
                            xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_auth_fail,time1, __icon__))
                            print("InControl Addon - Server Communication: Failed. Please check Addon settings - Server Username / Password")
                    except requests.exceptions.Timeout:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_timeout,time1, __icon__))
                        print("InControl Addon - Server Communication: Failed - Timeout")
                    except requests.exceptions.TooManyRedirects:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_redirects,time1, __icon__))
                        print("InControl Addon - Server Communication: Failed. Too Many Redirects")
                    except requests.exceptions.RequestException as e:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_fail,time1, __icon__))
                        print e

#Initialize Shortcut InControl Server Groups (Scenes)
shortActive = 0
shortActive1 = 0
shortActive2 = 0
shortActive3 = 0
...
class MyClass(xbmcgui.WindowDialog):
    ...
    def onControl(self, control):
        if control == self.button0:
            if (shortActive1 == 1):
                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = shortGid1 )
                if (str(settings.getSetting("short_1_win")) == "None"):
                    if (degug_mode == "Yes"):
                        ...
                        print("InControl Addon - Shortcut 1 Active - XBMC Window Not Set")
                else:
        ...
        if control == self.button1:
            if (shortActive2 == 1):
                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = shortGid2 )
                if (str(settings.getSetting("short_1_win")) == "None"):
        ...
        if control == self.button2:
            if (shortActive3 == 1):
                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = shortGid3 )
                if (str(settings.getSetting("short_1_win")) == "None"):
                    xbmc.executebuiltin('Notification(%s, Debug not completed, %d, %s)'%(__addonname__,time1, __icon__))
                else:
        ...
        if control == self.button3:
            if (shortActive4 == 1):
                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = shortGid4 )
                if (str(settings.getSetting("short_1_win")) == "None"):
                    xbmc.executebuiltin('Notification(%s, Debug not completed, %d, %s)'%(__addonname__,time1, __icon__))
                else:
        ...
        if control == self.button4:
            if (shortActive5 == 1):
                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = shortGid5 )
                if (str(settings.getSetting("short_1_win")) == "None"):
                    xbmc.executebuiltin('Notification(%s, Debug not completed, %d, %s)'%(__addonname__,time1, __icon__))
                else:
            ...
            if (str(settings.getSetting("audio_started")) == "Yes"):
                if (str(settings.getSetting("audio_start_gid")) == ""):
                    if (degug_mode == "Yes"):
                        ...
                        print("InControl Addon - Audio Started - Scene Active - ID Not Set")
                    else:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
                else:
                    if (night_mode == "Yes"):
                        if (night_switch == 1):
                            incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_started )
                        else:
                            if (degug_mode == "Yes"):
                                xbmc.executebuiltin('Notification(%s, Audio Started - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                print("InControl Addon - Audio Started - Scene Active - Night Mode On - Not Night")
                    else:
                        incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_started )
            else:
                if (degug_mode == "Yes"):
                    xbmc.executebuiltin('Notification(%s, Audio Started - Scene Inactive, %d, %s)'%(__addonname__,time1, __icon__))
                    print("InControl Addon - Audio Started - Scene Inactive")
            ...
            if (str(settings.getSetting("video_stopped")) == "Yes"):
                if (str(settings.getSetting("video_stop_gid")) == ""):
                    if (degug_mode == "Yes"):
                        ...
                        print("InControl Addon - Video Stopped - Scene Active - ID Not Set")
                    else:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
            	else:
            	    if (night_mode == "Yes"):
                        if (night_switch == 1):
                            incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_stopped )
                        else:
                            if (degug_mode == "Yes"):
                                xbmc.executebuiltin('Notification(%s, Video Stopped - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                print("InControl Addon - Video Stopped - Scene Active - Night Mode On - Not Night")
                    else:
                        incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_stopped )
            else:
                if (degug_mode == "Yes"):
                    xbmc.executebuiltin('Notification(%s, Video Stopped - Scene Inactive, %d, %s)'%(__addonname__,time1, __icon__))
                    print("InControl Addon - Video Stopped - Scene Inactive")
            ...
            if (str(settings.getSetting("audio_stopped")) == "Yes"):
                if (str(settings.getSetting("audio_stop_gid")) == ""):
                    if (degug_mode == "Yes"):
                        ...
                        print("InControl Addon - Audio Stopped - Scene Active - ID Not Set")
                    else:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
            	else:
            	    if (night_mode == "Yes"):
                        if (night_switch == 1):
                            incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_stopped )
                        else:
                            if (degug_mode == "Yes"):
                                xbmc.executebuiltin('Notification(%s, Audio Stopped - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                print("InControl Addon - Audio Stopped - Scene Active - Night Mode On - Not Night")
                    else:
                        incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_stopped )
            else:
                if (degug_mode == "Yes"):
                    xbmc.executebuiltin('Notification(%s, Audio Stopped - Scene Inactive, %d, %s)'%(__addonname__,time1, __icon__))
                    print("InControl Addon - Audio Stopped - Scene Inactive")
            ...
            if (str(settings.getSetting("video_ended")) == "Yes"):
                if (str(settings.getSetting("video_end_gid")) == ""):
                    if (degug_mode == "Yes"):
                        ...
                        print("InControl Addon - Video Ended - Scene Active - ID Not Set")
                    else:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
            	else:
            	    if (night_mode == "Yes"):
                        if (night_switch == 1):
                            incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_ended )
                        else:
                            if (degug_mode == "Yes"):
                                xbmc.executebuiltin('Notification(%s, Video Ended - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                print("InControl Addon - Video Ended - Scene Active - Night Mode On - Not Night")
                    else:
                        incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_ended )
            else:
                if (degug_mode == "Yes"):
                    xbmc.executebuiltin('Notification(%s, Video Ended - Scene Inactive, %d, %s)'%(__addonname__,time1, __icon__))
                    print("InControl Addon - Video Ended - Scene Inactive")
            ...
            if (str(settings.getSetting("audio_ended")) == "Yes"):
                if (str(settings.getSetting("audio_end_gid")) == ""):
                    if (degug_mode == "Yes"):
                        ...
                        print("InControl Addon - Audio Ended - Scene Active - ID Not Set")
                    else:
                        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
            	else:
            	    if (night_mode == "Yes"):
                        if (night_switch == 1):
                            incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_ended )
                        else:
                            if (degug_mode == "Yes"):
                                xbmc.executebuiltin('Notification(%s, Audio Ended - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                print("InControl Addon - Audio Ended - Scene Active - Night Mode On - Not Night")
                    else:
                        incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_ended )
            else:
                if (degug_mode == "Yes"):
                    xbmc.executebuiltin('Notification(%s, Audio Ended - Scene Inactive, %d, %s)'%(__addonname__,time1, __icon__))
                    print("InControl Addon - Audio Ended - Scene Inactive")
...
while( not xbmc.abortRequested ):
    if xbmcgui.Window( 10000 ).getProperty( "incontrolmenu" ) == "true":
        mydisplay = MyClass()
        mydisplay .doModal()
        del mydisplay
        xbmcgui.Window( 10000 ).setProperty( "incontrolmenu", "false" )
                ...
                if (str(settings.getSetting("video_paused")) == "Yes"):
                    if (VPAUSE == 0):
                        if (str(settings.getSetting("video_pause_gid")) == ""):
                            if (degug_mode == "Yes"):
                                ...
                                print("InControl Addon - Video Paused - Scene Active - ID Not Set")
                            else:
                                xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
                        else:
                            if (night_mode == "Yes"):
                                if (night_switch == 1):
                                    VPAUSE = 1
                                    incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_paused )
                                else:
                                    if (degug_mode == "Yes"):
                                        xbmc.executebuiltin('Notification(%s, Video Paused - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                        print("InControl Addon - Video Paused - Scene Active - Night Mode On - Not Night")
                            else:
                                VPAUSE = 1
                                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_paused )
                else:
                    if (VPAUSE == 0):
                        ...
                        if (degug_mode == "Yes"):
                            ...
                            print("InControl Addon - Video Paused - Scene Inactive")
                    ...
                    if (str(settings.getSetting("video_resumed")) == "Yes"):
                        if (str(settings.getSetting("video_resume_gid")) == ""):
                            if (degug_mode == "Yes"):
                                ...
                                print("InControl Addon - Video Resumed - Scene Active - ID Not Set")
                            else:
                                xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
                        else:
                            if (night_mode == "Yes"):
                                if (night_switch == 1):
                                    VPAUSE = 0
                                    incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_resumed )
                                else:
                                    if (degug_mode == "Yes"):
                                        xbmc.executebuiltin('Notification(%s, Video Resumed - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                        print("InControl Addon - Video Resumed - Scene Active - Night Mode On - Not Night")
                            else:
                                VPAUSE = 0
                                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_video_resumed )
                    else:
                        if (VPAUSE == 1):
                            ...
                            if (degug_mode == "Yes"):
                                ...
                                print("InControl Addon - Video Resumed - Scene Inactive")
        else:
            VIDEO = 0
            AUDIO = 1
                ...
                if (str(settings.getSetting("audio_paused")) == "Yes"):
                    if (APAUSE == 0):
                        if (str(settings.getSetting("audio_pause_gid")) == ""):
                            if (degug_mode == "Yes"):
                                ...
                                print("InControl Addon - Audio Paused - Scene Active - ID Not Set")
                            else:
                                xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
                        else:
                            if (night_mode == "Yes"):
                                if (night_switch == 1):
                                    APAUSE = 1
                                    incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_paused )
                                else:
                                    if (degug_mode == "Yes"):
                                        xbmc.executebuiltin('Notification(%s, Audio Paused - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                        print("InControl Addon - Audio Paused - Scene Active - Night Mode On - Not Night")
                            else:
                                APAUSE = 1
                                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_paused )
                else:
                    if (APAUSE == 0):
                        ...
                        if (degug_mode == "Yes"):
                            ...
                            print("InControl Addon - Audio Paused - Scene Inactive")
                    ...
                    if (str(settings.getSetting("audio_resumed")) == "Yes"):
                        if (str(settings.getSetting("audio_resume_gid")) == ""):
                            if (degug_mode == "Yes"):
                                ...
                                print("InControl Addon - Audio Resumed - Scene Active - ID Not Set")
                            else:
                                xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%(__addonname__,plugin_gid, time1, __icon__))
                        else:
                            if (night_mode == "Yes"):
                                if (night_switch == 1):
                                    APAUSE = 0
                                    incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_resumed )
                                else:
                                    if (degug_mode == "Yes"):
                                        xbmc.executebuiltin('Notification(%s, Audio Resumed - Scene Active - Night Mode On - Not Night, %d, %s)'%(__addonname__,time1, __icon__))
                                        print("InControl Addon - Audio Resumed - Scene Active - Night Mode On - Not Night")
                            else:
                                APAUSE = 0
                                incontrol_direct( ip = hip, port = hport, username = huser, password = hpword, command = g_audio_resumed )
                    else:
                        if (APAUSE == 1):
                            ...
                            if (degug_mode == "Yes"):
                                ...
                                print("InControl Addon - Audio Resumed - Scene Inactive")
